# Zayn
### Cloud Engineer · DevOps · AWS

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit_Website-000000?style=flat)](https://YOUR_PORTFOLIO_URL)

---

### About Me
I am a **Cloud Engineer** focused on AWS and Platform Engineering.

I build secure, automated infrastructure using **First Principles**. I combine **Infrastructure as Code** with **FinOps** strategies to engineer systems that are efficient by design. My goal is to master the craft of building production-grade platforms without unnecessary complexity or cost.

Currently executing a portfolio of infrastructure projects to demonstrate core engineering patterns.

### What I Build
*   **Immutable Infrastructure:** Baking security into AMIs with Packer & Ansible.
*   **Resilient Networks:** Architecting Multi-AZ VPCs
*   **Observability:** Monitoring with Prometheus & Grafana.
*   **Automation:** Self-hosted CI/CD runners and "Ops-in-a-Box" solutions.

---

### Featured Projects

| Project | Description | Stack |
| :--- | :--- | :--- |
| **AWS Compute Factory** | A hardened, automated pipeline for producing secure CI/CD build agents. Replaces expensive GitHub Runners with self-hosted, ephemeral EC2 instances. | `Packer` `Ansible` `AWS` `Bash` |
| **Cloud-Pulse** | "Ops-in-a-Box" monitoring solution. A self-contained observability stack deployed via Terraform and configured with Ansible. | `Terraform` `Docker` `Prometheus` `Grafana` |
| **ZAYN Books** | A Full-Stack DevSecOps demonstration. Secure supply chain pipeline with vulnerability scanning, signing, and E2E testing. | `Node.js` `React` `Trivy` `Cosign` `Playwright` |
| **HA-VPC Stack** | *(In Progress)* Production-grade AWS Network architecture. Multi-AZ, public/private segregation, and load balancing designed for high availability. | `Terraform` `AWS VPC` `ALB` |

---

### Tech Stack

**Infrastructure & Cloud**
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat&logo=terraform&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

**Automation & Config**
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat&logo=ansible&logoColor=white)
![Packer](https://img.shields.io/badge/Packer-02A8EF?style=flat&logo=packer&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnu-bash&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=github-actions&logoColor=white)

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![HCL](https://img.shields.io/badge/HCL-5F43E9?style=flat&logo=hashicorp&logoColor=white)
